...
const onPaymentAdded = async () => {
  if (selectedLoan.value) {
    try {
      payments.value = await getPaymentsByLoan(token, selectedLoan.value.id)
    } catch (error) {
      payments.value = []
    }
  }
  await loadLoans()
  if (selectedLoan.value) {
    const updatedLoan = loans.value.find(l => l.id === selectedLoan.value.id)
    if (updatedLoan) {
...
const handleDelete = async (id) => {
  ...
  try {
    ...
    if (selectedLoan.value && selectedLoan.value.id === id) {
      closeDetailsView()
    }
  } catch (error) {
...
<template>
  <div class="space-y-6">
        ...
        <div v-else>
          ...
          <div class="md:hidden space-y-3">
            <div
              ...
              :key="loan.id"
              class="rounded-2xl bg-slate-50 p-4 shadow-sm space-y-3 transition"
            >
              <div 
              ...
              >
                <div class="min-w-0">
                  <p class="font-semibold text-slate-900 text-sm truncate group-hover:text-blue-600 transition">{{ loan.name }}</p>
              ...
              <div class="flex items-center justify-between border-t border-slate-200 pt-3 mt-1">
                ...
                <div class="flex gap-3">
                  <button @click="openEditForm(loan)" class="text-xs font-semibold text-slate-600 transition hover:text-slate-900">Edit</button>
                  <button @click="handleDelete(loan.id)" class="text-xs font-semibold text-red-600 transition hover:text-red-700">Hapus</button>
                </div>
              </div>
            </div>
          </div>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-4 space-x-2">
                    <button
                      @click="openDetailsForLoan(loan)"
                      class="rounded-lg bg-blue-100 px-3 py-1 text-sm text-blue-700 transition hover:bg-blue-200"
                    >
                      View
                    </button>
                    <button
                      @click="openEditForm(loan)"
                      class="rounded-lg bg-slate-200 px-3 py-1 text-sm text-slate-700 transition hover:bg-slate-300"
                    >
                      Edit
                    </button>
                    <button
                      @click="handleDelete(loan.id)"
                      class="rounded-lg bg-red-100 px-3 py-1 text-sm text-red-700 transition hover:bg-red-200"
                    >
                      Hapus
                    </button>
                  </td>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="bg-white rounded-3xl p-6 shadow-lg">
        <LoanDetailsDialogContent :loan="selectedLoan" :payments="payments" @payment-added="onPaymentAdded" />
      </div>
    </div>
  </div>
</template>
