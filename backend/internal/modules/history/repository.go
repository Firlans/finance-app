...
func (r repository) Create(ctx context.Context, h *History) error {
	query := `INSERT INTO history (id, budget_id, amount, date, created_at) VALUES ($1, $2, $3, $4, $5)`
...
func (r repository) IsBudgetOwnedByUser(ctx context.Context, budgetID string, userID string) (bool, error) {
	var count int
	query := `SELECT count(1) FROM monthly_budgets WHERE id = $1 AND user_id = $2`

	err := r.db.QueryRow(ctx, query, budgetID, userID).Scan(&count)
	if err != nil {
		return false, err
	}
	return count > 0, nil
